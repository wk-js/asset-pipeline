...
function _tranformOutput(pipeline, asset, rule) {
    ...
    let options = {
        rule,
        input: Object.assign({ hash, fullpath: asset.input }, parse(asset.input)),
        output: Object.assign({ hash, fullpath: output }, parse(output))
    };
    rule.output = output = _rename(output, rule.output, options);
    options.output = Object.assign({ hash, fullpath: output }, parse(output));
 ...
 */
export function transform(pipeline, asset, rules) {
