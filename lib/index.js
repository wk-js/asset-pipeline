 ...
 *
 */
const _toUnixPath = function(pth) {
  pth = pth.replace(/\\/g, '/')
...
class AssetPipeline {
   ...
   */
  getPath( pth, baseDir ) {
    if (process.platform == 'win32') pth = path.normalize( pth )
    ...
    const output = _cleanPath( this.PREFIX + pth )

    return process.platform == 'win32' ? _toUnixPath( output ) : output
  }
